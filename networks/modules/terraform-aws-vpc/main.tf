...
resource "aws_route_table" "kafka" {
  count = local.create_vpc && var.create_kafka_subnet_route_table && length(var.kafka_subnets) > 0 ? 1 : 0
...
resource "aws_subnet" "kafka" {
  count = local.create_vpc && length(var.kafka_subnets) > 0 ? length(var.kafka_subnets) : 0
  ...
  tags = merge(
    {
      Name = try(
        var.kafka_subnet_names[count.index],
        format("${var.name}-${var.kafka_subnet_suffix}-%s", element(var.azs, count.index))
      )
    },
    var.tags,
...
resource "aws_network_acl" "kafka" {
  count = local.create_vpc && var.kafka_dedicated_network_acl && length(var.kafka_subnets) > 0 ? 1 : 0
...
resource "aws_network_acl_rule" "kafka_inbound" {
  count = local.create_vpc && var.kafka_dedicated_network_acl && length(var.kafka_subnets) > 0 ? length(var.kafka_inbound_acl_rules) : 0
...
resource "aws_network_acl_rule" "kafka_outbound" {
  count = local.create_vpc && var.kafka_dedicated_network_acl && length(var.kafka_subnets) > 0 ? length(var.kafka_outbound_acl_rules) : 0
